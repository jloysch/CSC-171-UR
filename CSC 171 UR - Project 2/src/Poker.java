...
public class Poker {
	...
	private ArrayList<Card> inputStrToCardArr(String str) {
		...
		for (int i = 0; i < str.length(); i+=2) {
			...
			//System.out.print("\tSUIT=" + str.charAt(i+1) + " && RANK=" + str.charAt(i) + "\n");
			
			Card c = new Card((this.inputStrToSuitIndex(String.valueOf(str.charAt(i+1)))),(this.inputStrToRankIndex(String.valueOf(str.charAt(i)))));
			TMP.add(c);
	...
	private int findNumberInString(String str) {
		...
		while (true) {
			try {
				if ( a > TMP.length) {
					//System.out.println("Couldn't find number in string '" + str + "'\n from sub-method returning >> " + tryBackToInt(str));
					break;
				}
	...
	public void addPoints(String CASE, int PlayerID) {
		...
		//System.out.println("Player " + (PlayerID+1) + " now has " + caseToPoints(CASE) + " points.");
	}
	...
	public void compareHands() {
		...
		for (int i = 0; i < this.PLAYERS.length; i++) {
			if (this.PLAYER_POINTS[i] > HIGHEST_POINTS_IN_INDEX) {
				...
				//System.out.println("Player " + (i+1) + " with " + this.PLAYER_POINTS[i] + " points.");
				WINNER = i;
			}
		}
	...
	private void calculatePairsKinds(int PlayerID) { //Also takes care of full house.
		...
		int PAIRS = 0, THREE_OF_A_KIND = 0; //Three of a kind is special, (full house).
		
		String TMP_STATS = "";
		
		for (int j = 0; j < CARD_FREQUENCIES.length; j++) { //Calculate pairs and kinds
			
			//System.out.println("CARD FREQUENCY FOR " + j + " = " + CARD_FREQUENCIES[j]);
		...
		//System.out.println("KINDS PAIRS >>\nKINDS = " + KINDS + "\n\tPAIRS = " + PAIRS);
		if ((PAIRS == 1) && (THREE_OF_A_KIND == 1)) {
			...
			for (int i = 0; i < tmp.length; i++) {
				if (!(tmp[i].equalsIgnoreCase("null") && !(tmp[i].equals(null)))) {
					//System.out.println("KINDS/E added " + tmp[i]);
				...
				for (int i = 0; i < 9; i++) {
					...
					for (int j = 0; j < 5; j++) {
						...
						if((matches==5)) { //break loop
							System.out.println("\t\t[*]-- Ascending pattern found for loops @ [" + i + "," + j + "]");
							fiveMatches = true;
							break;
						} else {
